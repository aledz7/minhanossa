...
if($_GET['pagina'] == '') {
	...
	$inicio = $_GET['pagina']*100;
	$limite = 100;
}

if($_GET['categoria'] <> ''){
	$sql .= "and (select count(1) FROM tbl_relaciona_categorias WHERE tbl_relaciona_categorias.id_produto = tbl_pecas.id and tbl_relaciona_categorias.id_categoria =  ".intval($_GET['categoria']).") > 0";
}

if($_GET['sub_categoria'] <> ''){
	$sql .= " and (select count(1) FROM tbl_relaciona_categorias WHERE tbl_relaciona_categorias.id_produto = tbl_pecas.id and tbl_relaciona_categorias.id_subcategoria =  ".intval($_GET['sub_categoria']).") > 0";
}

mysql_select_db($database_conexao, $conexao);
$query_rs_roupas = "SELECT * FROM tbl_pecas WHERE 1=1 $sql ORDER BY id DESC limit {$inicio},{$limite}";
