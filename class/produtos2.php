...
if($ProdutosInstanciada == '') {
	...
	class Produtos {
		...
		function rsDados($id='', $orderBy='', $limite='', $id_categoria='', $promocao='', $id_sub_categoria='', $id_op_especificacao='') {
			...
			if($id <> '') {
				$sql .= " and tbl_produtos.id = ?"; 
				$nCampos++;
				$campo[$nCampos] = $id;
			}
			
			if($promocao <> '') {
				$sql .= " and tbl_produtos.promocao = ?"; 
				$nCampos++;
				$campo[$nCampos] = $promocao;
			}
			
			if($id_categoria <> '') {
				$sql .= " and tbl_produtos.id_categoria = ?"; 
				$nCampos++;
				$campo[$nCampos] = $id_categoria;
			}
			
			if($id_sub_categoria <> '') {
				$sql .= " and tbl_produtos.subcategoria = ?"; 
				$nCampos++;
			...
			if($id_op_especificacao <> '') {
				...
				$joins .= " left join tbl_relaciona_produto_outras_especificacoes on tbl_produtos.id = tbl_relaciona_produto_outras_especificacoes.id_produto";
				$nCampos++;
				$campo[$nCampos] = $id_op_especificacao;
			}
			
			if($_GET['busca'] <> '' and $_GET['acao'] == 'buscar') {
				$sql .= " and (tbl_produtos.nome like ? or tbl_categorias.titulo like ? or tbl_produtos.detalhes like ?)"; 
			}
			...
			if($this->exibeRelacaoFiltroNome <> '') {
				foreach($this->exibeRelacaoFiltroNome as $i => $nome) {
					...
					$joins .= " 
						left join tbl_relaciona_filtros as tbl_rel_filtro_{$nome} on tbl_produtos.id = tbl_rel_filtro_{$nome}.id_produto and tbl_rel_filtro_{$nome}.id_menuFiltro = {$this->exibeRelacaoFiltroIdMenu[$i]} 
						left join tbl_noticias as tbl_filtro_{$nome} on tbl_rel_filtro_{$nome}.id_filtro = tbl_filtro_{$nome}.id";
					$labels_adicionais .= ", tbl_filtro_{$nome}.titulo as nome_{$nome}, tbl_filtro_{$nome}.breve as breve_{$nome}, tbl_filtro_{$nome}.por as por_{$nome}, tbl_filtro_{$nome}.id as id_{$nome}";
				}
			}
			...
			if($orderBy <> '') {
				...
				if($orderBy == 'mais_vendidos') {
					$sqlOrdem = " order by (select count(1) from tbl_pedidos_por_id_compra inner join tbl_compras on tbl_pedidos_por_id_compra.id_compra = tbl_compras.id where tbl_produtos.id = tbl_pedidos_por_id_compra.produto) desc";
				}
				
				if($orderBy == 'novidades') {
					$sqlOrdem = " order by tbl_produtos.id desc";
				}
				
				if($orderBy == 'vCrescente') {
					$sqlOrdem = " order by tbl_produtos.preco_por asc";
				}
				
				if($orderBy == 'vDecrescente') {
					$sqlOrdem = " order by tbl_produtos.preco_por desc";
				}
			}
			...
			foreach($this->filtros as $filtro) {
				if($filtro == 'lista_desejos') {
					$sql = " and (select count(1) from tbl_lista_desejos where tbl_lista_desejos.id_cliente = '{$_SESSION['dadosLogadoCLiente']->id}' and tbl_lista_desejos.id_produto = tbl_produtos.id) > 0";
				}
			}
			
			//// Especificações
			try{   
				$sql_especificacoes = "select * from tbl_noticias where idMenu = 3 and tipo = 'conteudos'";
				$stm = $this->pdo->prepare($sql_especificacoes);
				$stm->execute();
				$result_especificacoes = $stm->fetchAll(PDO::FETCH_OBJ);
				if(count($result_especificacoes) > 0) {
					foreach($result_especificacoes as $item_especificacoes) { 
						$labels_adicionais .= " ,tbl_nome_esp{$item_especificacoes->id}.titulo as nome_especificacao{$item_especificacoes->id}";
						$joins .= " left join tbl_relaciona_produto_outras_especificacoes as tbl_especicacao{$item_especificacoes->id} on tbl_produtos.id = tbl_especicacao{$item_especificacoes->id}.id_produto and tbl_especicacao{$item_especificacoes->id}.id_especificacao = {$item_especificacoes->id} left join tbl_noticias as tbl_nome_esp{$item_especificacoes->id} on tbl_nome_esp{$item_especificacoes->id}.id = tbl_especicacao{$item_especificacoes->id}.id_opcao_especificacao";
					}
				}
			...
			try{   
				$sql = "SELECT tbl_produtos.*, tbl_categorias.titulo as nome_categoria, tbl_sub_categorias.titulo as nome_subcategoria $labels_adicionais FROM tbl_produtos left join tbl_noticias as tbl_categorias on tbl_produtos.id_categoria = tbl_categorias.id left join tbl_noticias as tbl_sub_categorias on tbl_produtos.subcategoria = tbl_sub_categorias.id $joins where 1=1 $sql $sqlOrdem $sqlLimite";
		...
		function rs_categorias($tipo='categoria', $acao='') {
			...
			if($acao == 'mais_categorias') {
				...
				$labels_adicionais .= ", (select count(1) from tbl_produtos where tbl_produtos.{$campo_filtro} = tbl_noticias.id) as total_produtos_cat, (select tbl_produtos.foto from tbl_produtos where tbl_produtos.{$campo_filtro} = tbl_noticias.id limit 0,1) as foto_produto";
				$sql .= " and (select count(1) from tbl_produtos where tbl_produtos.{$campo_filtro} = tbl_noticias.id) > 0"; 
			}
...
<div class="form-group">
		...
		function add() {
			if($_POST['acao'] == 'addProdutos') {
				...
				try{   
					$sql = "INSERT INTO tbl_produtos (nome, id_categoria, detalhes, peso, foto, preco_por, preco_de, ativo, breve_detalhes, subcategoria, estoque, codigo, mostrar_preco, venda_minima, itens_inclusos, especificacoes, fornecedor, promocao, palavras_chaves, altura, largura, comprimento, preco_de_atacado, preco_por_atacado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
					$stm = $this->pdo->prepare($sql);   
