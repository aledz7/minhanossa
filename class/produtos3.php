...
if($ProdutosInstanciada == '') {
	...
	class Produtos {
		...
		function rsDados($id='', $orderBy='', $limite='', $id_categoria='', $promocao='', $id_sub_categoria='', $id_op_especificacao='') {
			...
			if($id <> '') {
				$sql .= " and tbl_produto.id = ?"; 
				$nCampos++;
				$campo[$nCampos] = $id;
			}
			
			if($promocao <> '') {
				$sql .= " and tbl_produto.promocao = ?"; 
				$nCampos++;
				$campo[$nCampos] = $promocao;
			}
			
			if($id_categoria <> '') {
				$sql .= " and tbl_produto.id_categoria = ?"; 
				$nCampos++;
				$campo[$nCampos] = $id_categoria;
			}
			
			if($id_sub_categoria <> '') {
				$sql .= " and tbl_produto.subcategoria = ?"; 
				$nCampos++;
			...
			if($id_op_especificacao <> '') {
				...
				$joins .= " left join tbl_relaciona_produto_outras_especificacoes on tbl_produto.id = tbl_relaciona_produto_outras_especificacoes.id_produto";
				$nCampos++;
				$campo[$nCampos] = $id_op_especificacao;
			}
			
			if($_GET['busca'] <> '' and $_GET['acao'] == 'buscar') {
				$sql .= " and (tbl_produto.nome like ? or tbl_categorias.titulo like ? or tbl_produto.detalhes like ?)"; 
			}
			...
			if($orderBy <> '') {
				...
				if($orderBy == 'mais_vendidos') {
					$sqlOrdem = " order by (select count(1) from tbl_pedidos_por_id_compra inner join tbl_compras on tbl_pedidos_por_id_compra.id_compra = tbl_compras.id where tbl_produto.id = tbl_pedidos_por_id_compra.produto) desc";
				}
				
				if($orderBy == 'novidades') {
					$sqlOrdem = " order by tbl_produto.id desc";
				}
				
				if($orderBy == 'vCrescente') {
					$sqlOrdem = " order by tbl_produto.preco_por asc";
				}
				
				if($orderBy == 'vDecrescente') {
					$sqlOrdem = " order by tbl_produto.preco_por desc";
				}
			}
			...
			try{   
				$sql = "SELECT tbl_produto.*, tbl_categoria.categoria as nomeCategoria FROM tbl_produto left join tbl_categoria on tbl_produto.categoria = tbl_categoria.id where 1=1 $sql $sqlOrdem $sqlLimite";
		...
		function rs_categorias($tipo='categoria', $acao='') {
			...
			if($acao == 'mais_categorias') {
				...
				$labels_adicionais .= ", (select count(1) from tbl_produto where tbl_produto.{$campo_filtro} = tbl_noticias.id) as total_produtos_cat, (select tbl_produto.foto from tbl_produto where tbl_produto.{$campo_filtro} = tbl_noticias.id limit 0,1) as foto_produto";
				$sql .= " and (select count(1) from tbl_produto where tbl_produto.{$campo_filtro} = tbl_noticias.id) > 0"; 
			}
