...
if($_GET['pagina'] == '') {
	...
	$inicio = intval($_GET['pagina'])*100;
	$limite = 100;
}

if($_GET['categoria'] <> ''){
	$sql .= "and (select count(1) FROM tbl_relaciona_categorias WHERE tbl_relaciona_categorias.id_produto = tbl_acervo.id and tbl_relaciona_categorias.id_categoria = ".intval($_GET['categoria']).") > 0";
}

if($_GET['sub_categoria'] <> ''){
	$sql .= " and (select count(1) FROM tbl_relaciona_categorias WHERE tbl_relaciona_categorias.id_produto = tbl_acervo.id and tbl_relaciona_categorias.id_subcategoria =  ".intval($_GET['sub_categoria']).") > 0";
}

mysql_select_db($database_conexao, $conexao);
$query_rs_roupas = "SELECT * FROM tbl_acervo WHERE 1=1 $sql ORDER BY id DESC limit {$inicio},{$limite}";
...
?>
	 <div class="col-sm-4 foto">
		<div class="thumbnail">
			<a href="desc-nosso-acervo.php?id=<?php echo $row_rs_roupas['id']?>">
				<?php $img_reduz_peca = "minhanossa/img_noticias/".cortaImagem($row_rs_roupas['foto'], 'minhanossa/img_noticias', '360', '500', 'img_reduz_peca', '#FFFFFF');?>
			...
			<div class="caption deg-produto-rosa text-left caption-color" style="padding: 53px 10px 0px;">
				<!--<small>Titulo</small> -->
				<a href="add-lista-desejos.php?id_produto=<?php echo $row_rs_roupas['id'];?>"><h5 style="color: #FFFFFF;">Clique na foto e descubra mais</h5></a>
