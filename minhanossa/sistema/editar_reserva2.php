...
if((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "formEditcontrato")) {
	...
	$updateSQL = sprintf("UPDATE tbl_contrato SET loja=%s, vendedor=%s, data_evento=%s, data_devolucao=%s, codigo_cliente=%s, nome_cliente=%s, tipo_contrato=%s, comentario=%s, tipo_plano=%s WHERE id=%s",
...
buscaGenericad('codigo_cliente', 'id', '', 'Clientes', 'nome', "parent.document.getElementById('envia').src='busca-plano.php?id=[id]';", 'tbl_cliente', $concatCampos, $where);?>
                ...
                Selecione
                <?php do{?>
                <option value="<?php echo $row_rs_planos['id'];?>" <?php if($row_rs_contrato['tipo_plano'] == $row_rs_planos['id']){ echo "selected";}?>/><?php echo utf8_decode($row_rs_planos['nome']);?>
                <?php }while($row_rs_planos = mysql_fetch_assoc($rs_planos));?>
                                ...
                                <div class="col-md-4">
                                	...
                                	<a href="imprimir-aluguel.php?id=<?php echo $_GET['id'];?>&tipo=D" class="btn btn-primary btn-mini"> <i class="iconfa-ok"></i>&nbsp; Imprimir Devolu&ccedil;&atilde;o</a>
                                </div>
