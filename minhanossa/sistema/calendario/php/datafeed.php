  ...
  try{
    ...
    //echo($sql);
      ...
      $ret['Msg'] = mysql_error();
		...
		$idCompromisso = mysql_insert_id();
		
		$insertSQL = sprintf("INSERT INTO tbl_relaciona_agenda_usuario (id_usuario, id_compromisso) VALUES (%s, %s)",
					   GetSQLValueString($_SESSION['idFuncionario'], "text"),
					   GetSQLValueString($idCompromisso, "text"));
		$Result1 = mysql_query($insertSQL) or die(mysql_error());
		
		$ret['IsSuccess'] = true;
		$ret['Msg'] = 'Compromisso registrado com sucesso.';
		$ret['Data'] = $idCompromisso;
	}
	}catch(Exception $e){
     $ret['IsSuccess'] = false;
...
function listCalendarByRange($sd, $ed){
  ...
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "select jqcalendar.* from `jqcalendar` where `starttime` between '"
...
function updateCalendar($id, $st, $et){
  ...
  try{
    ...
    //echo $sql;
      ...
      $ret['IsSuccess'] = true;
      $ret['Msg'] = 'Compromisso atualizado com sucesso';
    }
	}catch(Exception $e){
     $ret['IsSuccess'] = false;
     $ret['Msg'] = $e->getMessage();
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $tz){
  ...
  try{
    ...
    $sql = "update `jqcalendar` set"
      . " `starttime`='" . php2MySqlTime(js2PhpTime($st)) . "', "
      . " `endtime`='" . php2MySqlTime(js2PhpTime($et)) . "', "
      . " `subject`='" . texto(utf8_decode(mysql_real_escape_string($sub))) . "', "
      . " `isalldayevent`='" . mysql_real_escape_string($ade) . "', "
      . " `description`='" . utf8_decode(mysql_real_escape_string($dscr)) . "', "
      . " `location`='" . utf8_decode(mysql_real_escape_string($loc)) . "', "
...
function removeCalendar($id){
  ...
  try{
    ...
    $sql = "delete from `jqcalendar` where `id`=" . $id;
      ...
      $ret['IsSuccess'] = true;
      $ret['Msg'] = 'Compromisso excluido com sucesso';
    }
	}catch(Exception $e){
     $ret['IsSuccess'] = false;
     $ret['Msg'] = $e->getMessage();
