...
function getCalendarByRange($id){
	try{
		...
		$handle = mysql_query($sql, $conexao) or die(mysql_error());
		//echo $sql;
...
<html xmlns="http://www.w3.org/1999/xhtml" >
  <head>    
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
           ...
           $("#stpartdate,#etpartdate").datepicker({ picker: "<button class='calpick'></button>"});    
            ...
            $("#calendarcolor").colorselect({ title: "Cor", index: cv, hiddenid: "colorvalue" });
	...
	a.imgbtn span.arquivos {
	  background: url(./css/images/arquivos.png) no-repeat 3px 4px;
	}
	
	a.imgbtn span.enviarEmail {
	  background: url(./css/images/mail.png) no-repeat 3px 4px;
	}
    </style>
  </head>
  <body>    
    <div>      
      <div class="toolBotton">           
        <a id="Savebtn" class="imgbtn" href="javascript:void(0);">                
          <span class="Save" title="Salvar alterações.">Salvar</span>          
        </a>                           
        <?php if(isset($event)){ ?>
        ...
        <a id="Closebtn" class="imgbtn" href="javascript:void(0);">                
          <span class="Close" title="Fechar janela." >Fechar</span></a>            
        </a>        
      </div>                  
      ...
      <div class="infocontainer">            
        <form action="php/datafeed.php?method=adddetails<?php echo isset($event)?"&id=".$event->Id:""; ?>" class="fform" id="fmEdit" method="post">                 
          <label>                    
            ...
            <input MaxLength="200" class="required safe" id="Subject" name="Subject" style="width:85%; padding-left:3px; margin-left:4px;" type="text" value="<?php echo isset($event)?$event->Subject:"" ?>"  />                     
          ...
          <label>                    
            ...
            <div>  
              <?php if(isset($event)){
				  ...
				  $dataDe = substr($sarr[0],3,2).'/'.floatval(substr($sarr[0],0,2)).'/'.substr($sarr[0],6,4);
				  
                  $earr = explode(" ", php2JsTime(mySql2PhpTime($event->EndTime)));
				  $dataAte = substr($earr[0],3,2).'/'.floatval(substr($earr[0],0,2)).'/'.substr($earr[0],6,4);
              }?>                    
              <input MaxLength="10" class="required date" id="stpartdate" name="stpartdate" style="padding-left:2px;width:90px;" type="text" value="<?php echo isset($event)?$dataDe:""; ?>" />                       
              <input MaxLength="5" class="required time" id="stparttime" name="stparttime" style="width:40px;" type="text" value="<?php echo isset($event)?$sarr[1]:""; ?>" /> &agrave;                        
              <input MaxLength="10" class="required date" id="etpartdate" name="etpartdate" style="padding-left:2px;width:90px;" type="text" value="<?php echo isset($event)?$dataAte:""; ?>" />                       
              ...
              <label class="checkp"> 
                <input id="IsAllDayEvent" name="IsAllDayEvent" type="checkbox" value="1" <?php if(isset($event)&&$event->IsAllDayEvent!=0) {echo "checked";} ?>/>     O dia inteiro                     
              </label>                    
            </div>                
          </label>                 
          <label>                    
...
<textarea cols="20" id="Description" name="Description" rows="2" style="width:95%; height:180px">
