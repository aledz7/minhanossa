...
$.Window = (function()  {
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			maxWidth: 1800,                // [number:800] the maximum width, if -1 means no checking
			maxHeight: 1800,               // [number:600] the maximum height, if -1 means no checking
		...
		function initialize(instance) {
			...
			// add close button
			if( options.closable ) {
				headerFuncPanel.append( "<div title='Fechar' class='closeImg window_icon_button no-draggable'></div>" );
			...
			if( options.minimizable ) {
				headerFuncPanel.append( "<div title='Minimizar' class='minimizeImg window_icon_button no-draggable'></div>" );
			...
			if( options.url != null && $.trim(options.url) != "" ) {
				...
				// show loading image
				container.append("<div class='frame_loading'>Carregando...</div>");
